...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="form-group">
                <label>貼文類型</label>
                <select id="post-type">
                    <option value="auto">自動（依星期排班）</option>
                    <option value="product">商品介紹</option>
                    <option value="opinion">觀點文（Claude 生成）</option>
                    <option value="wishlist">許願互動文（Claude 生成）</option>
                </select>
            </div>
            <div class="btn-group">
    ...
    <script>
        ...
        async function loadLog() {
            try {
                ...
                box.innerHTML = data.logs.map(l => {
                    ...
                    const typeLabel = l.post_type ? ` [${l.post_type}]` : '';
                    return `<div class="${cls}">${icon} [${l.time}]${typeLabel} ${l.title} → ${l.platforms}</div>`;
                }).join('');
        ...
        async function postNow() {
            ...
            try {
                const res = await fetch(`${BASE_URL}/api/post?count=${count}&platforms=${platforms.join(',')}&post_type=${postType}`);
                const data = await res.json();
...
# ============================================
webhook_log = []  # [{'time', 'title', 'platforms', 'success', 'post_type'}]
MAX_LOG = 50
...
def append_log(title, platforms_result, post_type='product'):
    """記錄發文結果"""
...
def handle_new_product_async(product):
    ...
    # Webhook 新商品永遠用 product 類型，確保商品資訊完整曝光
    content = build_post_content(product, config, post_type='product')
    platforms = ['fb', 'ig', 'threads']
    results = post_to_platforms(content, platforms, config)

    append_log(title, results, post_type='product')
...
def api_post():
    ...
    # 貼文類型：auto = 依星期排班；否則直接用指定類型
    post_type_param = request.args.get('post_type', 'auto')
    if post_type_param == 'auto':
        post_type = get_today_post_type()
    else:
        post_type = post_type_param
    print(f"[api_post] 貼文類型：{post_type}")
    ...
    for i in range(count):
        ...
        results = post_to_platforms(content, platforms, config)
        append_log(product.get('title', ''), results, post_type=post_type)
        posted.append({
...
def post_smart():
    ...
    # 支援 ?post_type=opinion|wishlist|product|auto
    post_type_param = request.args.get('post_type', 'auto')

    def do_post():
        post_type = get_today_post_type() if post_type_param == 'auto' else post_type_param
        print(f"[post_smart] 貼文類型：{post_type}")
        ...
        for i in range(count):
            ...
            for platform, result in results.items():
                status = "✅" if result.get('success') else "❌"
                print(f"[post_smart] {status} {platform}")
